...
def plot_update_query_throughput_cms():
    sketch_width = 272
    sketch_depth = 5
    ...
    for type in ["dynamic", "countmin", "elsatic"]:
        ...
        s_per_million_update = ms_per_update * 100
        throughput_update = 1 / s_per_million_update
        ms_per_query = np.array(result['log_query_time'].dropna().to_numpy()).mean()
        s_per_million_query = ms_per_query * 100
        throughput_query = 1 / s_per_million_query
        print(type, throughput_query, throughput_update)
...
def plot_update_query_throughput(B: int, L: int, figure_name: str):
    sketch_width = 272
    sketch_depth = 5
    ...
    for l in range(L):
        for b in range(2, B):
            ...
            ms_per_update = np.array(result['log_update_time'].dropna().to_numpy()).mean()
            s_per_million_update = ms_per_update * 100
            throughputs_update[l, b - 2] = 1 / s_per_million_update
            ms_per_query = np.array(result['log_query_time'].dropna().to_numpy()).mean()
            s_per_million_query = ms_per_query * 100
            throughputs_query[l, b - 2] = 1 / s_per_million_query
    im0 = ax0.imshow(throughputs_update, vmin=5, vmax=28, origin='lower', extent=[2, B, 0, L])
    im1 = ax1.imshow(throughputs_query, vmin=5, vmax=28, origin='lower', extent=[2, B, 0, L])
    fig.subplots_adjust(right=0.8)
    cbar_ax = fig.add_axes([0.85, 0.15, 0.05, 0.7])
    fig.colorbar(im0, cax=cbar_ax, label="MOPS")
...
def plot_gs_undo_expand(B: int, L: int, max_cycles: int, granularity: int, count_log: int, figure_name: str):
    sketch_width = 272
    sketch_depth = 5
    packets_per_log = COUNT_PACKETS // count_log
...
def plot_gs_dynamic_undo_comparison(B: int, L: int, count_log: int, figure_name: str):
    sketch_width = 272
    sketch_depth = 5
    packets_per_log = COUNT_PACKETS // count_log
    ...
    for j, command_lines0 in enumerate([command_geometric, command_countmin]):
        ...
        ax0.plot(x, y_mem, label=f'{names_type[j]}-{j}', marker=markers[j])
        ax1.plot(x, y_mae, label=f'{names_type[j]}-{j}', marker=markers[j])
...
if __name__ == "__main__":
    # plot_gs_dynamic_undo_comparison(2, 5, 32, "plot_gs_dynamic_undo_comparison")
    # plot_mae_dynamic_and_linked_cell(4, 128)
    # plot_mae_cellsketch_expand(2, 128)
    # plot_update_query_throughput_cms()
