...
long GeometricSketchMH::getLastVectorIndexFromKey(
    uint32_t key,
    uint32_t row_id
) {
...
long GeometricSketchMH::getFirstAllocatedVectorIndexFromKey(uint32_t key, uint32_t row_id, int& prev_layer_id, int& prev_layer_row_index, int& prev_counter_row_index, int& prev_B_pow)
{
    multihash.initialize(key, row_id);
    const int O = compressed_counters;
    prev_layer_id = 0;
    prev_layer_row_index = 0;
    prev_counter_row_index = multihash.first();
    prev_B_pow = 1;
    const int max_vector_index = counters.size() + compressed_counters - 1;
...
long GeometricSketchMH::getNextVectorIndexFromKey(
    uint32_t key,
    uint32_t row_id,
    int& prev_layer_id,
...
/*
TEST_SUITE("GeometricSketchMH Helper Methods") {
    constexpr auto W = 10;
    constexpr auto D = 2;
    constexpr auto B = 2;
    GeometricSketchMH gs(W, D, B);
    ...
    TEST_CASE("getNextVectorIndexFromKey") {
        ...
        MultiHash multihash;
        multihash.setFirstSubHashModulus(W);
        multihash.setSubHashModulus(B);

        uint64_t hash_0 = multihash.first();
        uint64_t hash_1 = multihash.next();
        uint64_t hash_2 = multihash.next();
        uint64_t hash_3 = multihash.next();
...
TEST_SUITE("GeometricSketchMH Core Methods") {
    ...
    TEST_CASE("undoExpand") {
        ...
        uint64_t key = 0;
        GeometricSketchMH regular_gs(W, D, B);
        GeometricSketchMH undone_gs(W, D, B);
